...
def get_client_stream_requests():
    while True:
        ...
        hello_request = gchat_pb2.ChatRequest(greeting = "Hello", name = name)
        yield hello_request
        time.sleep(1)
...
def run():
    with grpc.insecure_channel('localhost:50000') as channel:
        ...
        if rpc_call == "1":
            ...
            print("MutualStream")
            responses = stub.MutualStream(get_client_stream_requests())

            for response in responses:
                print("MutualStream Response Received: ")
                print(response)
